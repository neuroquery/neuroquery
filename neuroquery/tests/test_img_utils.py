...
def test_get_masker():
    ...
    assert np.allclose(np.diag(masker.mask_img_.affine)[:3], [-2., 2., 2.])
...
def test_coords_to_voxels():
    ...
    coords = img.affine.dot(
        (np.hstack([voxels, np.ones((len(voxels), 1))]).T)).T[:, :-1]
...
def test_gaussian_coord_smoothing():
    coords = [(0., 0., 0.), (10., -10., 30.)]
    computed_img = img_utils.gaussian_coord_smoothing(coords)
    masker = NiftiSpheresMasker(coords + [(-10., 10., -30)]).fit()
    values = masker.transform(computed_img)[0]
    assert (values[:2] > computed_img.get_data().max() / 2.).all()
