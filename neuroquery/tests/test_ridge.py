...
def test_z_maps(regressor):
    x, y = datasets.make_regression(
        n_features=9, n_targets=13, effective_rank=7, bias=10, n_informative=5,
        shuffle=False, noise=5, n_samples=57, random_state=1)
    test_feat = [1, 3, 7]
...
def test_predictions(regressor):
    x, y = datasets.make_regression(
        n_samples=102, n_informative=5, n_features=91, n_targets=117,
        effective_rank=9, noise=.5, shuffle=False, random_state=0)
    reg = regressor()
    sk_reg = RidgeCV()
